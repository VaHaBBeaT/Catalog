...
namespace Catalog
{
    public partial class form_CreateFilm : Form
    {
        ...
        private void btn_SaveBook_Click(object sender, EventArgs e)
        {
            ...
            Film film = parse.Find(f => f.filmProducer == txtbox_Producer.Text && f.filmTitle == txtbox_Title.Text && f.filmPremiere == DateTime.Parse(txtbox_Premiere.Text).Date);
            if (film == null)
            {
                ...
                try
                {
                    ...
                    tssl_StatusFilmCreate.Text = "Фильм успешно добавлен!";
                ...
                catch (FileNotFoundException ex)
                {
                    ...
                    tssl_StatusFilmCreate.Text = "Ошибка!";
                }
            }
            else
            {
                MessageBox.Show(string.Format("Фильм '{0}' режиссера '{1}' уже существует, необходимо исправить информацию", txtbox_Producer.Text, txtbox_Title.Text));
            }
        }
            ...
            if (ofd_FileSelector.ShowDialog() == DialogResult.OK)
            {
                foreach (string file in ofd_FileSelector.FileNames)
                {
                    try
                    {
                        ...
                        tssl_StatusFilmCreate.Text = "Файл добавлен!";
                    }
